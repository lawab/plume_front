...
class RapportList extends ConsumerStatefulWidget {
  const RapportList({
    Key? key,
  }) : super(key: key);

  @override
  ConsumerState<RapportList> createState() => RapportListState();
}

class RapportListState extends ConsumerState<RapportList> {
  ...
  Widget build(BuildContext context) {
    ...
    final viewModele = ref.watch(getDataReportsFuture);
    if (check == false) {
        ...
        if (viewModel.listStudent[i].repo == true) {
          for (int j = 0; j < viewModele.listReports.length; j++) {
            if (viewModel.listStudent[i].reports!
              ...
              if (viewModele.listReports[j].validated == false) {
