...
import 'package:projet_plum/pages/services/getModule.dart' as m;
...
class ModifHomePage extends ConsumerStatefulWidget {
  ...
  @override
  ModifHomePageState createState() => ModifHomePageState();
}

class ModifHomePageState extends ConsumerState<ModifHomePage> {
  ...
  Future<void> _loadFromAssets() async {
    try {
      final doc = Document.fromJson(jsonDecode(widget.module.content!));
      setState(() {
        _controller = QuillController(
            document: doc, selection: const TextSelection.collapsed(offset: 0));
      });
    } catch (error) {
      final doc = Document()..insert(0, 'Bonjour,');
      setState(() {
        _controller = QuillController(
            document: doc, selection: const TextSelection.collapsed(offset: 0));
      });
  ...
  Widget horizontalView(double height, double width, BuildContext contextt) {
    return AppLayout(
      content: Column(children: [
        Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          kIsWeb
              ...
              : Container(height: 70, child: toolbar),
          const SizedBox(
            height: 5,
          ),
          Expanded(
  ...
  Future dialogText(BuildContext context, content) {
    return showDialog(
        context: context,
        builder: (con) {
          return AlertDialog(
            ...
            title: const Center(
              child: Text(
                "Saisissez les derniers détails et enregistrer",
                style: TextStyle(
                  color: Colors.black,
                  fontWeight: FontWeight.bold,
              ...
              ElevatedButton.icon(
                ...
                onPressed: () {
                  ...
                  if (titleController.text.isNotEmpty &&
                    ...
                    modificationModule(context, titleController.text,
                        descriptionController.text, content, widget.module.sId);
                    Navigator.pop(con);
                  } else {
                    showTopSnackBar(
                      Overlay.of(context),
  ...
  //////////////////////////////////////////////////////////////////////////////////////////  SERVICES
  Future<void> modificationModule(
      BuildContext contextt, nom, description, content, idModule) async {
...
// "creator": id,
    ...
    try {
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        ref.refresh(m.getDataModulesFuture);
        ...
        if (role == 'SUDO' || role == 'ADMIN') {
          Navigator.push(
              contextt, MaterialPageRoute(builder: (context) => Module()));
