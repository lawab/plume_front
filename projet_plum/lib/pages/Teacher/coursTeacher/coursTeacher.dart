...
class CoursTeacher extends ConsumerStatefulWidget {
  const CoursTeacher({
    Key? key,
  }) : super(key: key);

  @override
  ConsumerState<CoursTeacher> createState() => CoursTeacherState();
}

class CoursTeacherState extends ConsumerState<CoursTeacher> {
  ...
  Widget build(BuildContext context) {
    ...
    if (filtre == false) {
      for (int i = 0; i < viewModelCat.listCategorie.length; i++) {
        for (int s = 0; s < viewModel.listcoursvalideTeacher.length; s++) {
          List<Courses> dummySearchList = [];

          if (viewModelCat.listCategorie[i].courses!
              .contains(viewModel.listcoursvalideTeacher[s].sId)) {
            dummySearchList.add(viewModel.listcoursvalideTeacher[s]);
        ...
        builder: (context, constraints) {
          if (constraints.maxWidth >= 900) {
            return horizontalView(height(context), width(context), context,
                listCours, viewModel.listcoursTotale);
          } else {
                                                            ...
                                                            child: RichText(
                                                              text: TextSpan(
                                                                children: [
                                                                  ...
                                                                  TextSpan(
                                                                    text: listCours[index].courses![indexx].validated ==
                                                                            true
                                                                        ? 'Validé'
                                                                        : 'Non validé',
                                                                    style: TextStyle(
                                                                        ...
                                                                        fontSize:
                                                                            15,
                                                                        color: listCours[index].courses![indexx].validated ==
                                                                                true
                                                                            ? Colors
                                                                                .green
                                                                            : Colors
                                ...
                                child: GridView.builder(
                                  ...
                                  gridDelegate:
                                      const SliverGridDelegateWithMaxCrossAxisExtent(
                                          ...
                                          mainAxisSpacing: 50,
                                          mainAxisExtent: 270),
                                  itemCount: CoursSearch[index].courses!.length,
                                  itemBuilder: ((context, indexx) {
                                    return InkWell(
                                      child: Card(
                                        shape: RoundedRectangleBorder(
