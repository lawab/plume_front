...
class Signaler extends ConsumerStatefulWidget {
  final Students eleve;
  final List<Courses> list;

  const Signaler({Key? key, required this.eleve, required this.list})
      : super(key: key);

  @override
  ConsumerState<Signaler> createState() => SignalerState();
}

class SignalerState extends ConsumerState<Signaler> {
  @override
  void initState() {
  ...
  Widget build(BuildContext context) {
    if (check == false) {
      for (int i = 0; i < widget.list.length; i++) {
        if (idTeacher == widget.list[i].course!.creator!.sId) {
          listCours.add(widget.list[i]);
          check = true;
        }
      }
    }
                        ...
                        child: Column(
                          children: [
                            ...
                            DropdownButtonFormField(
                              ...
                              items: listCours.map((Courses val) {
                                return DropdownMenuItem(
                                  value: val.course!.sId,
                                  child: Text(
                                    val.course!.title!,
                                  ),
                                );
                    ...
                    Expanded(
                      child: ElevatedButton(
                        ...
                        onPressed: () {
                          ...
                          serviceEcart(context, cours, widget.eleve.user!.sId!,
  ...
  Future<void> serviceEcart(
      BuildContext contextT, idCourse, studentId, motif, gravity) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
